...
def compute_mrr(y_true: np.ndarray, y_score: np.ndarray) -> float:
    ...
    # Find the max relevance label in this list
    max_label = np.max(y_true)
    
    # Find rank of first item with max_label
...
def evaluate_ranking(
    ...
    y_scores is a dict mapping model_name -> predicted_scores
    """
    ...
    for q, indices in grouped.items():
        if len(indices) < 2:
            # Cannot rank single items meaningfuly or ndcg might complain if only 1 item
            # But we can still compute them formally. ndcg_score handles shape (1, n).
            pass

        current_y_true = y_true[indices]
        
        # Skip if all labels are the same (no relevance distinction possible)
        if np.unique(current_y_true).size < 2 and np.max(current_y_true) == 0:
             # If all are 0, no relevant items. 
             pass
        ...
        for model_name, scores_full in y_scores.items():
            ...
            # nDCG@5
            # sklearn ndcg_score expects shape (n_samples, n_features) -> (1, n_items)
...
def main():
    ...
    # 5. Ranking Evaluation (Test Set Only)
    print("\n--- Ranking Evaluation (Test Set) ---")
    
    # Prepare scores dictionary including baseline
    # Ensure baseline scores are comparable or just raw; nDCG/MRR only care about relative order
    ranking_scores = {
